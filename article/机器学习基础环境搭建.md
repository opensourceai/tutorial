...
最后,按下**ESC**键，输入**`:`**,然后输入**`wq`**按下回车,保存退出.输入`source /etc/profile`重新载入配置文件.
...
对于 TensorFlow 及深度学习的业余爱好者或者初学者， [Visual Studio Code](https://code.visualstudio.com/)或者一些在线的交互式 Python 环境（比如免费的 [Google Colab](https://colab.research.google.com/) ）也是不错的选择。Colab 的使用方式可参考[附录](https://tf.wiki/zh/appendix/cloud.html#colab) 。
...
1. PyTorch
   ...
   进入[PyTorch官网]( https://pytorch.org/ ),选择合适的安装方式.

   ![image-20191106011523361](img/image-20191106011523361.png)

2. PaddlePaddle

   进入[PaddlePaddle官网]( https://www.paddlepaddle.org.cn/install/quick ),选择合适的安装方式.
